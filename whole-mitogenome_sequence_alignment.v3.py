    ...
    for i in range(0, len(Input_file_content), 1):
        ...
        for a in range(0, len(Input_content), 1):
            ...
            if data_line[0] != data_line[1]:
                ...
                #做了多次测试80是较好的选择
                if float(data_line[2]) > 80.0 and float(data_line[10]) <= 1e-30:
        ...
        for id in q_align_4_lie_max_line:
            ...
            select_start = int(max_start) - 10000
            select_end = int(max_end + 10000)
        ...
        for id in select:
            ...
            # 如果ID对应有多行
                ...
                for g in range(0, len(select[id]), 1):
                    ...
                    # 如果字典中存在,并且片段与字典中比对上的长度总和小于（block_shared_length - 10)；这一段也保留
                    elif len_line.get(id, 'no_value') != 'no_value' and (length_int + len_line[id]) <= (
                            block_shared_length - 10):
                        len_line[id] += length_int
                        QQ.write(select[id][g])

                    # 如果与字典中长度相加在参考block长度上下浮动10,这一段也要，然后停止
                    ...
                    # ========== 核心新增逻辑：累加超过+30时，跳过当前片段（不写入）并继续下一个 ==========
                    elif len_line.get(id, 'no_value') != 'no_value' and (length_int + len_line[id]) > (
                            block_shared_length + 30):
                        # 不写入当前片段，直接跳过，继续遍历下一个片段
                        continue

                    # 原有逻辑：累加和>目标+10但≤+30时，写入并终止
                    elif len_line.get(id, 'no_value') != 'no_value' and (
                            block_shared_length + 30) > (length_int + len_line[id]) > (
                            block_shared_length + 10):
                        QQ.write(select[id][g])
                        break
        QQ.close()
